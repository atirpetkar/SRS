...
class FlashcardPayload(BaseModel):
    ...
    def validate_string_lists(cls, v):
        if v is not None and not all(isinstance(item, str) and len(item.strip()) > 0 for item in v):
...
class MCQOption(BaseModel):
    ...
    rationale: str | None = Field(default=None, description="Explanation for this option")
...
class MCQPayload(BaseModel):
    ...
    multiple_select: bool | None = Field(default=False, description="Allow multiple correct answers")
...
class ClozeBlank(BaseModel):
    ...
    alt_answers: list[str] | None = Field(default=None, description="Alternative acceptable answers")
    case_sensitive: bool | None = Field(default=False, description="Whether matching is case sensitive")
    ...
    def validate_alt_answers(cls, v):
        if v is not None and not all(isinstance(ans, str) and len(ans.strip()) > 0 for ans in v):
...
class ShortAnswerExpected(BaseModel):
    ...
    unit: str | None = Field(default=None, description="Expected unit (for numeric answers)")


class ShortAnswerGrading(BaseModel):
    """Grading configuration for short answer."""
    
    method: str = Field(default="exact", description="Grading method: exact, regex, numeric")
...
class ShortAnswerPayload(BaseModel):
    ...
    acceptable_patterns: list[str] | None = Field(default=None, description="Regex patterns for acceptable answers")
    grading: ShortAnswerGrading = Field(default_factory=ShortAnswerGrading, description="Grading configuration")
...
class FlashcardValidator:
    ...
    def validate(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            raise ValueError(f"Invalid flashcard payload: {e}")
...
class MCQValidator:
    ...
    def validate(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            raise ValueError(f"Invalid MCQ payload: {e}")
...
class ClozeValidator:
    ...
    def validate(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            raise ValueError(f"Invalid cloze payload: {e}")
...
class ShortAnswerValidator:
    ...
    def validate(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            raise ValueError(f"Invalid short answer payload: {e}")
