...
from api.v1.review.fsrs import fsrs_state_from_db, fsrs_state_to_db_dict, FSRSState
...
async def get_review_queue(
    ...
    for item in due_items:
        ...
        due_queue.append(QueueItemResponse(
            id=item.id,
            type=item.type,
    ...
    for item in new_items:
        new_queue.append(QueueItemResponse(
            id=item.id,
            type=item.type,
            render_payload=item.payload,
            due_at=None,
...
async def record_review(
    ...
    item_query = select(Item).where(and_(
    ...
    state_query = select(SchedulerState).where(and_(
        SchedulerState.user_id == principal.user_id,
    ...
    interval_days = (final_state.due_at - datetime.now(timezone.utc)).days
