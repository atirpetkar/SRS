...
down_revision: Union[str, Sequence[str], None] = 'e81ea979110f'
...
def upgrade() -> None:
    ...
    sa.Column('created_at', postgresql.TIMESTAMP(timezone=True), server_default=sa.text('now()'), nullable=False),
    sa.Column('deleted_at', postgresql.TIMESTAMP(timezone=True), nullable=True),
    sa.CheckConstraint("difficulty IS NULL OR difficulty IN ('intro', 'core', 'stretch')", name='items_difficulty_check'),
    sa.CheckConstraint("status IN ('draft', 'published')", name='items_status_check'),
    sa.ForeignKeyConstraint(['org_id'], ['orgs.id'], ),
    ...
    op.create_index('items_tags_gin', 'items', ['tags'], unique=False, postgresql_using='gin')
